...
1. **M3 repurposing validation — Percival's next attack** — 9 candidates (crizotinib, dabrafenib, cyclosporine, sirolimus, tacrolimus, estradiol, cetrorelix, sulfasalazine, dinoprostone). Input: `docs/data/repurposing_candidates.json` + `docs/research/evidence/digest-2026-09-02.md`. DoD: per-candidate depth-check (mechanism fit, ChEMBL/PubMed evidence, safety, novelty) → ranked shortlist with confidence tags → `docs/research/evidence/` + knowledge pages + llms.txt update. *Owner: Percival*
