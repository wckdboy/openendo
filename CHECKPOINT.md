...
3. **M1 fold-input → AlphaFold run** — 35 novel targets packaged (`docs/research/structures/fold_input/`); **27/35 already in AFDB → only 8 need folding (~$1–3)**. Needs compute decision (RunPod serverless vs MacBook M3 Pro local vs Colab) + cost estimate. DoD: folding run launched, or explicit deferral with reason logged. *Owner: Jaeger (decide) / Percival (prepare)*
...
|----|-------|--------|--------------|-------|
| M1 | Fold-input pack (35 novel targets, AlphaFold-ready) | ✅ merged · ⏭ 8 to fold | 2026-09-02 | Percival |
...
- [ ] **T7 ELN interviews** — interview guide (DK) exists; Phase A (agent-del): shortlist 2–3 danske labs/institutter + outreach-udkast (DK); Phase B: interviews (menneske). DoD: shortlist + outreach-udkast i `docs/research/`. *Percival — næste efter M3 close-out*
...
- **AlphaFold compute budget** — 27/35 in AFDB; 8 to fold ≈ $1–3 on serverless. Decision still open (top-3 #3). *Jaeger (decide)*
